...
func exportDir(ctx android.BaseContext) []string {
	...
	if ctx.DeviceConfig().TargetUsesGralloc1() {
		export_include_dirs = append(export_include_dirs, "gralloc_legacy")
	} else {
		export_include_dirs = append(export_include_dirs, "gralloc")
	}
